    ...
    S: ServerImplementation,
{
    ...
    #[inline]
    pub fn stream<U>(self, stream: U) -> Server<S, H, U>
    where
...
/// An empty stream that does not produce any messages.
...
impl Sender {
    ...
    where
        T: Encode<Binary> + api::Broadcast,
    {
        let message = BroadcastMessage::new(message)?;
