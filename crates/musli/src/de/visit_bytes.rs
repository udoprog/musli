...
///
/// This behaves the same as [`Visit`], but implicitly hints that the caller is
/// after bytes.
///
...
///
/// [`Visit`]: super::Visit
pub trait VisitBytes<'de, M = DefaultMode> {
    /// Decode the given input using a closure as visitor.
    fn visit_bytes<D, F, O>(cx: &D::Cx, decoder: D, f: F) -> Result<O, <D::Cx as Context>::Error>
    where
        D: Decoder<'de, Mode = M>,
