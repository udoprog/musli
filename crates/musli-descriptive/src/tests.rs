...
fn pack_max() {
    ...
    assert_eq!(Tag::from_byte(bytes[3]), Tag::new(Kind::Bytes, 62));
    assert_eq!(bytes.len(), 4 + MAX_INLINE_LEN);
}
...
fn pow2() {
    ...
    assert_eq!(Tag::from_byte(bytes[3]), Tag::new(Kind::Pack, 7));
    assert_eq!(bytes.len(), SIZE + 22);
    assert!(&bytes[SIZE + 4..].iter().all(|b| *b == 0));
}
